...
(function () {
  ...
  // Inject the floating booking widget into every page
  // The widget visibility can be controlled from the CRM's Website Manager
  function injectWidget() {
    var widget = document.createElement('div');
    widget.id = 'kingsmen-booking-widget';
    ...
    link.setAttribute('aria-label', 'Book a haircut at The Kingsmen Barbers');
    link.style.cssText = [
      'display:flex',
      'align-items:center',
      'gap:8px',
      ...
      'box-shadow:0 4px 20px rgba(191,155,81,0.35)',
      'transition:transform 0.2s,box-shadow 0.2s',
      'white-space:nowrap',
    ].join(';');
    link.innerHTML = '<span aria-hidden="true">✂</span> Book Now';
    link.addEventListener('mouseenter', function () {
      link.style.transform = 'scale(1.05)';
      link.style.boxShadow = '0 6px 28px rgba(191,155,81,0.45)';
    });
    link.addEventListener('mouseleave', function () {
      link.style.transform = 'scale(1)';
      link.style.boxShadow = '0 4px 20px rgba(191,155,81,0.35)';
    });
  ...
  // Check CRM setting for widget visibility; fall back to showing it
  fetch(API_BASE + '/api/public/settings/booking_widget_enabled')
    .then(function (res) {
      if (!res.ok) return { value: 'true' };
      return res.json();
    })
    .then(function (data) {
      var enabled = !data || data.value !== 'false';
      if (enabled) injectWidget();
    })
    .catch(function () {
      // If API is unreachable, show widget by default
      injectWidget();
    });
